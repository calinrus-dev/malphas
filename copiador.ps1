...
$btnStruct.Text = "COPY STRUCT (Folder Tree Only)"
...
$btnAll.Text = "COPY ALL (All Code + Paths)"
...
$btnStruct.Add_Click({
    ...
    [System.Windows.Forms.MessageBox]::Show("Structure copied to clipboard.", "Malphas Core")
})
...
$btnAll.Add_Click({
    ...
    if ([string]::IsNullOrEmpty($dump)) {
        [System.Windows.Forms.MessageBox]::Show("No valid files found.", "Error")
    } else {
        [System.Windows.Forms.Clipboard]::SetText($dump)
        [System.Windows.Forms.MessageBox]::Show("All code copied to clipboard.", "Malphas Core")
    }
})
