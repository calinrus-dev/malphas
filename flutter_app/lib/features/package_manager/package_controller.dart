...
class PackageController extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        final demoMhp = File('${packagesDir.path}/bouncing_demo.mhp');
        final demoMsp = File('${packagesDir.path}/bouncing_demo.msp');
        final demoMeta =
            File('${packagesDir.path}/bouncing_demo.manifest.json');

        if (!demoMhp.existsSync() ||
            !demoMsp.existsSync() ||
            !demoMeta.existsSync()) {
          try {
            final mhpBytes =
                await rootBundle.load('assets/packages/bouncing_demo.mhp');
            await demoMhp.writeAsBytes(mhpBytes.buffer
                .asUint8List(mhpBytes.offsetInBytes, mhpBytes.lengthInBytes));
      ...
      if (examplesDir.existsSync()) {
        for (final entity in examplesDir.listSync(recursive: true)) {
          if (entity is File && entity.path.toLowerCase().endsWith('.mhp')) {
            final result = _parseMhpPackage(entity);
            if (result != null) {
      ...
      for (final file in packagesDir.listSync()) {
        if (file is File && file.path.toLowerCase().endsWith('.mhp')) {
          final result = _parseMhpPackage(file);
          if (result != null) {
            final isLoaded = persistedIds.contains(result.package.id);
            final pack = EntityPackage(
              result.package.id,
  ...
  /// Parses a valid MLPH binary and returns a [_ParsedPackageResult] containing relational tables.
  _ParsedPackageResult? _parseMhpPackage(File file) {
    try {
      ...
      for (int i = 0; i < entitiesCount; i++) {
        final entryOffset = entitiesTableOffset + (i * 32);
  ...
  /// but its `.mhp` artifact is missing.
